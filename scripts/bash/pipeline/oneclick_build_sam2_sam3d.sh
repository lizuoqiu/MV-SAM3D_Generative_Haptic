...
#
# Usage:
#   bash scripts/bash/pipeline/oneclick_build_sam2_sam3d.sh <dataset_root> [options]
...
if [[ "${SKIP_PREPARE}" != "1" ]]; then
  ...
  python "${ROOT_DIR}/scripts/python/data/prepare_dataset_for_mvsam3d.py" --dataset-root "${DATASET_ROOT}"
else
...
SAM2_REPO_DIR="${SAM2_REPO_DIR:-${ROOT_DIR}/sam2d}" \
  "${ROOT_DIR}/scripts/bash/pipeline/run_sam2_batch_masks.sh" "${DATASET_ROOT}" "${IMAGES_SUBDIR}" "${MASK_DIR}" "${CUTOUT_DIR}"

log "Step 3/3: Running MV-SAM3D reconstruction (resume mode)"
if [[ -n "${EXCLUDE_OBJECT}" ]]; then
  "${ROOT_DIR}/scripts/bash/pipeline/run_mvsam3d_remaining_batch.sh" "${DATASET_ROOT}" "${MASK_DIR}" "${STAGE2_WEIGHT}" "${EXCLUDE_OBJECT}"
else
  "${ROOT_DIR}/scripts/bash/pipeline/run_mvsam3d_remaining_batch.sh" "${DATASET_ROOT}" "${MASK_DIR}" "${STAGE2_WEIGHT}"
fi
