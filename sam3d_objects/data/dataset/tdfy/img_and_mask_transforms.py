...
class ObjectCentricSSI(SSIPointmapNormalizer):
    ...
    def _compute_scale_and_shift(self, pointmap: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        logger.debug(
            f"[ObjectCentricSSI] pointmap_size={pointmap_size}, "
            f"mask_shape={tuple(mask.shape)}, resized_mask_shape={tuple(mask_resized.shape)}, "
            f"num_mask_pixels={num_mask_pixels}, num_mask_points={num_mask_points}, "
